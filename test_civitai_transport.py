...
class CivitAITransportTests(unittest.TestCase):
    ...
    def test_validates_each_redirect_hop(self, request):
        redirect = FakeResponse(redirect=True, location="https://image.civitai.com/file/model.safetensors")
        terminal = FakeResponse()
        request.side_effect = [redirect, terminal]

        response = civitai_transport.civitai_request("GET", "https://civitai.com/api/download/models/1")
    ...
    def test_rejects_untrusted_redirect_before_following_it(self, request):
        ...
        with self.assertRaises(ValueError):
            civitai_transport.civitai_request("GET", "https://civitai.com/api/download/models/1")
